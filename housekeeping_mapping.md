...
The closed-building crew takes an exclusive lock. Nobody gets in or out. It builds a new compact building with the same tenants, fresh filing cabinets, a clean status board, and reopens at the correct size. It removes the bloat completely, in exchange for a full lockout while it runs. In PostgreSQL this is `VACUUM FULL` and its near-twin `CLUSTER` (the same rewrite, tenants written in a chosen order). A coming in-core command, `REPACK`, folds the two under one name — and it is this crew: it locks exactly as they do, which is a trap waiting in the upcoming major version: For about fifteen years "repack" has meant the *online* extension. Core has now taken the same word for the traditional rewrite that takes a full table lock for the entire operation's duration. So `REPACK` the built-in and `pg_repack` the extension are about to sit on opposite sides of the key axis in this context: one shuts the building, the other doesn't.
