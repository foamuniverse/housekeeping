...
The closed-building crew takes an exclusive lock. Nobody gets in or out. It builds a new compact building with the same tenants, fresh filing cabinets, a clean status board, and reopens at the correct size. It removes the bloat completely, in exchange for a full lockout while it runs. In PostgreSQL this is `VACUUM FULL` and its near-twin `CLUSTER` (the same rewrite, tenants written in a chosen order). A coming in-core command, `REPACK`, folds the two under one name — and it is this crew: it locks exactly as they do.

That name is a trap to defuse in advance. For about fifteen years "repack" has meant the *online* extension below — lock-free, safe to cron. Core has now taken the same word for the *locking* rewrite. So `REPACK` the built-in and `pg_repack` the extension are about to sit on opposite sides of the only axis that matters: one shuts the building, the other doesn't. Read "Postgres has `REPACK` now," assume the behaviour of the repack you already know, schedule it at noon — and you take a full outage.
...
In PostgreSQL this is `pg_repack` and `pg_squeeze` — extensions, not core. An in-core open-building crew (a concurrent mode for `REPACK`) has been worked toward but is still aspirational and far from certain in any given release. For now, the online rebuild of a *table* lives outside core.

The dangers are the price of staying open. Two buildings run at once, so space roughly doubles. The swap needs a moment of exclusive access; if a visitor at the front desk won't leave, it waits, and if it waits too long the crew gives up and retries. The worst case is the crew killed mid-operation — power loss, crash, abort: the original building survives and no tenants are harmed, but the relay is still wired into its front desk adding overhead to every change, and the half-built replacement sits on the lot consuming space until cleared. And the relay has a standing cost the closed crew doesn't — the variants that capture changes through logical decoding (`pg_squeeze`, and any in-core concurrent successor) hold a replication slot for the whole rebuild, which pins the campus registration horizon for as long as the job runs.

### The filing cabinets are the exception

One part of "rebuild" is already solved online, in core, today: the cabinets themselves. `REINDEX CONCURRENTLY` builds a fresh cabinet beside the old one and swaps it in under a lock that blocks neither reads nor writes — the machinery of `CREATE INDEX CONCURRENTLY` — then drops the bloated original. Compacting a *cabinet* needs no extension and no window; only rebuilding the *building* does. Keep the two apart, because "bloat" collapses them and their remedies are at different stages: the index half has a smooth in-core online command now, the table half is still extension-or-aspirational.
