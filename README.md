PostgreSQL's VACUUM is one command that does four unrelated things — reclaiming space, updating planner statistics, maintaining the visibility map, and freezing transaction IDs against wraparound — named after only one of them. The name points operators at cleanup when cleanup is often not the problem.
