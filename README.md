...
PostgreSQL's VACUUM is one command that does two unrelated things — reclaiming space and freezing transaction IDs against wraparound — named after only one of them, maintaining a shared data structure (the visibility map) as it goes. The autovacuum daemon runs it automatically, along with a third job, ANALYZE, which updates the planner's statistics. Three jobs, one misleading name: "vacuum" says cleanup, but only one of the three is cleanup, and when something goes wrong the name points you at a space-reclaiming function that might have nothing to do with the problem. This repo builds a university-campus housekeeping metaphor that gives each job its own name, so the three stop collapsing into one and reasoning about them comes more naturally.
