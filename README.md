...
By suggesting cleanup, `VACUUM` by itself misdirects naive problem solvers in situations where reclaiming space is not the issue at hand. Documents in this repo build a university campus housekeeping metaphor that intends to make the four functions legible to help tell the four functions apart.
...
*housekeeping_mapping.md* — the campus model mapped to the actual system: the four functions, the autovacuum dispatcher and workers, the failsafe vacuum, manual vacuum, repack, the failure modes, and the configuration parameters in campus terms.
